...
extension Park {
    var accentColor: Color {
        switch shortName {
        case "Magic Kingdom":     return Color(red: 0.55, green: 0.35, blue: 0.95)
        case "EPCOT":             return Color(red: 0.30, green: 0.65, blue: 1.00)
        case "Hollywood Studios": return Color(red: 1.00, green: 0.55, blue: 0.25)
        case "Animal Kingdom":    return Color(red: 0.35, green: 0.85, blue: 0.55)
...
// Single Canvas draw of all stars — one view instead of 110
private struct StarfieldCanvas: View {
    private struct Star {
        let x: CGFloat
        let y: CGFloat
        let radius: CGFloat
        let phase: Double  // offset so stars don't all twinkle in sync
    }
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1/20)) { timeline in
            Canvas { ctx, size in
                ...
                for star in stars {
                    let brightness = 0.4 + 0.6 * (0.5 + 0.5 * sin(t * 1.2 + star.phase))
                    let rect = CGRect(
...
private struct SparkleAccent: View {
    ...
    var body: some View {
        Canvas { ctx, size in
            let center = CGPoint(x: size.width * 0.88, y: size.height * 0.18)
            for i in 0..<8 {
                ...
                ctx.stroke(path, with: .color(color.opacity(0.35)), lineWidth: 1.8)
            }
            ctx.fill(
